...
impl SvnApp {
    ...
    /// 后台检查一次版本更新（读服务端 latest.json，与目录无关的任务）。
    /// 地址有改动时会顺带保存配置。
    pub fn spawn_update_check(&mut self) {
        if self.cfg.update_server.trim().is_empty() {
        ...
        self.pool
            .spawn(Kind::CheckUpdate, usize::MAX, "检查更新".into(), move |sink| {
                sink.line(format!("$ 检查版本更新：{server}"));
                match update::check(&server) {
                    Ok(manifest) => {
                        sink.line(format!("→ 服务端最新版本：V{}", manifest.version.trim()));
                        Data::UpdateCheck {
                            ok: true,
                            message: String::new(),
                            info: Some(manifest),
            ...
            move |sink| {
                ...
                match update::download(&url, &dest) {
                    Ok(bytes) => {
                        ...
                        // 防御无限更新循环：服务端版本号升了但 exe 没换（或换回了同一个文件）
                        ...
                        if let Ok(self_exe) = std::env::current_exe() {
                            match (update::sha256_of(&dest), update::sha256_of(&self_exe)) {
                                (Ok(new_hash), Ok(cur_hash)) if new_hash == cur_hash => {
                                    ...
                                    return Data::UpdateDownloaded {
                                        ok: false,
                                        message: "下载的文件与当前程序完全相同：服务端发布的 exe 可能没有换成新构建，已取消覆盖".to_owned(),
                                        bytes,
                                    };
                                }
                                _ => {}
    ...
    /// `mine` 为真时只取本机 svn 登录人的记录（`svn log --search`）。
    /// `unlimited` 为真时临时不带 `-l`（不限条数拉全量），只影响这一次读取，不写设置。
    pub fn spawn_log(&mut self, index: usize, mine: bool, unlimited: bool) {
        let (Some(svn), Some(path)) = (self.svn_or_none(), self.dir_path(index)) else {
            return;
        };
        ...
        // limit 传 0 表示不限条数（svn.log 里据此省掉 -l）
        let limit = if unlimited { 0 } else { self.cfg.log_limit.clamp(1, 2000) };
        ...
        self.pool
            .spawn(Kind::Log, index, format!("{label} 提交记录"), move |sink| {
                ...
                sink.line(format!(
                    "$ svn log -v -r HEAD:1{limit_part}{shown} --xml \"{}\"",
                    path.display()
                ));
                let (entries, run) = svn.log(&path.to_string_lossy(), limit, &author);
    ...
    pub fn open_history(&mut self, index: usize) {
        ...
        self.history = Some(HistoryPage::new(index, label, limit, user, only_mine));
        self.page = Page::History;
        let mine = self.history.as_ref().is_some_and(|page| page.mine);
        self.spawn_log(index, mine, false);
    }
            ...
            match (kind, data) {
                ...
                (Kind::AuthUser, Data::User { user }) => {
                    ...
                    if let Some(dir) = again {
                        let unlimited = self.history.as_ref().is_some_and(|page| page.unlimited);
                        self.spawn_log(dir, true, unlimited);
                    }
                }
                (Kind::CheckUpdate, Data::UpdateCheck { ok, message, info }) => {
                    if !ok {
                        ...
                        match info {
                            ...
                            None => self.push(Level::Warning, "服务端没有返回版本信息"),
                        }
                    }
                }
                ...
                (Kind::Update, Data::Run { dir, ok, message, .. }) => {
                    ...
                    let mine = self.history.as_mut().filter(|h| h.dir == dir).map(|h| {
                        h.entries.clear();
                        h.picked = None;
                        h.mine
                    });
                    if let Some(mine) = mine {
                        let unlimited = self.history.as_ref().is_some_and(|h| h.unlimited);
                        self.spawn_log(dir, mine, unlimited);
                    }
                }
    ...
    fn tick(&mut self, ctx: &egui::Context) {
        ...
        // 启动后到点的第一次版本更新检查：
        // 只在配置了服务端地址且开关打开时查，地址为空就安静跳过（不催用户配置）
        if let Some(at) = self.update_check_at {
            if Instant::now() >= at {
                self.update_check_at = None;
                if self.cfg.check_update_on_start && !self.cfg.update_server.trim().is_empty() {
            ...
            .show(ctx, |ui| {
                ScrollArea::vertical().id_salt("settings").show(ui, |ui| {
                    ...
                    ui.collapsing("常规", |ui| {
                        ...
                        } else if self.cfg.update_server.trim().is_empty() {
...
mod tests {
    use super::*;
    use egui::{RawInput, ThemePreference};
